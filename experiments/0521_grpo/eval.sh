...
experiment_name="exp_short_prompt_${graph_spec}_model_${model_name}_bs${batch_size}_lr${lr}_roll${n_rollout}_p${max_prompt_length}_r${max_response_length}_ts${train_size}_rwd${reward_fn}"
...
python3 -u $(realpath ../../)/lib/scripts/main_eval.py \
    ...
    custom_reward_function.path=prepare_ppo_data.py > logs/${experiment_name}.${id}.eval.out 2> logs/${experiment_name}.${id}.eval.err
