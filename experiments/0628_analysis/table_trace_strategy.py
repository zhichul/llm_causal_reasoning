...
folder = 'tables/trace_strategy/'
os.makedirs(folder, exist_ok=True)
d = load_yaml('configs/table_trace_strategy.yaml')
...
for column in d['breakdowns']:
    ...
    for row in d['lines']:
        ...
        for i in range(len(generations)):
            ...
            if 'other' in column['name'] and '7b' in row['name'] and score == 1:
                print(example['responses'][0])
                print(results[i]['annotations'][0])
                input("")
            raw_scores.append(score)
    keep, pvals, means = indistinguishable_from_best(rows_raw,higher_is_better=True)
    keeps.append(keep)
    columns_values.append((means * 100).tolist())
    column_names.append(f"\shortstack[c]{{{column['name']}\\\\(n={len(raw_scores)})}}")

make_latex_table(column_names, row_names, columns_values, keeps, f'tables/trace_strategy/table.tex')
