...
def generate_observational_examples(config: addict.Dict, eval_uniqueness_check=True):
    np.random.seed(config.seed)

    if config.model.type == "gaussian_relu":
        data_generator = gaussian_relu.observational
    elif config.model.type == "discrete_cpt":
        data_generator = discrete_cpt.observational
    else:
        ...
        for n_vars in n_vars_sweep:
            ...
            for n_vals in n_vals_sweep:
                ...
                for i, dag in tqdm.tqdm(split_graphs):
                    ex, g, sample = data_generator.generate_observational_example_group(config, 
...
if __name__ == '__main__':
    ...
    if config.type != "observational":
        raise ValueError(f"Did you have a typo? Running `observational` data generation on a config file of type: {config.type}")
    
    os.makedirs(config.out_dir, exist_ok=True)
    copy_file(args.config, os.path.join(config.out_dir, 'config.yaml'))
    generate_observational_examples(config)